...
export function createPlayingCards(
    ...
    const colliders = [
        ...cardsData.map((card, i) => ({
            ...
            offset: { x: baseX + card.dx, y: -2.745 + i * 0.002, z: baseZ + card.dz },
            rotation: { y: card.angle },
        })),
        {
            type: 'box',
            halfExtents: [width / 2, deckHeight / 2, height / 2],
            offset: { x: deckX, y: deckY, z: deckZ },
            rotation: { y: -0.3 },
        },
    ];
    ...
    return { ...result, interact, dispose: result.dispose };
}
