...
const FUDGE_SIDES = DIE_SHAPE_FACE_COUNT[DIE_TYPE_CATALOG.dF?.shape ?? 'd6'];
...
class Parser {
    ...
    consume(): string {
        ...
        if (tok == null) throw new NotationError('Unexpected end of expression');
        return tok;
    }
    ...
    parseDiceToken(tok: string): DiceGroup {
        ...
        const sidesRaw = m[2];
        if (sidesRaw == null) throw new NotationError(`Invalid dice term "${tok}"`);
