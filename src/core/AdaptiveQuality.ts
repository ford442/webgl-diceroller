...
export function guessInitialQualityProfile(rendererState: any) {
    const params = new URLSearchParams(window.location.search);
...
export function refineQualityProfileFromProbe(initialProfile: any, probe: any) {
...
export function createFrameProbe(scheduler: any, { durationMs = 1000 }: any = {}) {
    let startedAt: number | null = null;
    let totalMs = 0;
    let sampleCount = 0;
    let done = false;
    let result: { avgFrameMs: number; sampleCount: number; durationMs: number } | null = null;

    return {
        /** @param {{ time?: number }} [frameState] */
        update(frameState: { time?: number } = {}) {
...
export function setGodRaysVisible(scene: any, visible: any) {
 ...
 */
export function setAccentLightsEnabled(scene: any, enabled: any) {
    scene.userData.accentLightRig?.setEnabled?.(enabled);
}

export function applyShadowLightPolicy(scene: any, policy: any) {
    scene.traverse((child: any) => {
 ...
 */
export function applyMotionProfile(active: any, deps: any) {
...
export function startPostLoadAdaptiveProbe(state: any, scheduler: any) {
...
export function updateAdaptiveQualityProbe(probe: any, context: any, state: any) {
 ...
 * }} deps
 */
export function updateMotionProfileState(deps: any) {
