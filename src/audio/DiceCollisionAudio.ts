...
export function createDiceCollisionAudio(options: any = {}) {
    ...
    function createSpatialBus(position: any) {
    ...
    function releaseSpatialBus(bus: any, delaySec: any) {
        if (!bus?.dispose) return;
        window.setTimeout(() => bus.dispose(), Math.max(10, delaySec * 1000));
    }

    function acquireVoice(durationSec: any) {
    ...
    function makeNoiseBuffer(seconds: any, decay: any = true) {
        const ctx = audioContext;
    ...
    function selectVoice(event: any) {
    ...
    function synthNoiseTone(
        now: any,
        output: any,
        { filterType, freq, q, noiseGain, noiseDecay, toneType, toneFreq, toneGain, toneDecay }: any
    ) {
        const ctx = audioContext;
    ...
    function synthMetalRing(
        ...
        const ctx = audioContext;
        const dest = output ?? masterGain;
        partials.forEach((ratio: any, i: any) => {
    ...
    function synthVoice(
        voice: any,
        { volume, impactBias, massBias = 0, sidesPitch = 1, position = null }: any
    ) {
    ...
    function playFluteNote(startAt: any, freq: any, dur: any, vol: any) {
        const ctx = audioContext;
