 ...
 */
export function createDiceJailController(deps: any) {
    ...
    /** @type {Set<number>} */
    const heldWasmIds = new Set<number>();
    ...
    const hold = (idsOrNearby = 'nearby') => {
        ...
        const targets =
            idsOrNearby === 'nearby'
                ...
                : spawnedDice.filter(
                      (die) =>
                          die.wasmId != null &&
                          Array.isArray(idsOrNearby) &&
                          idsOrNearby.includes(die.wasmId)
                  );
        ...
        targets.forEach((die, index) => {
            ...
            if (die.wasmId != null) heldWasmIds.add(die.wasmId);
        });
...
export function registerDiceJailController(controller: any) {
