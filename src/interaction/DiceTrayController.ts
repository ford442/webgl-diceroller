 ...
 */
export function createDiceTrayController(deps: any) {
    ...
    /** @type {Set<number>} */
    const lockedWasmIds = new Set<number>();

    const setHighlightVisible = (visible: boolean) => {
    ...
    const lock = () => {
        ...
        dice.forEach((die) => {
            ...
            if (die.wasmId != null) lockedWasmIds.add(die.wasmId);
        });
...
export function registerDiceTrayController(controller: any) {
