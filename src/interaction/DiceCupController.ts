 ...
 */
export function createDiceCupController(deps: any) {
    const {
        cupProp,
        camera,
        beginCupRoll,
    ...
    const projectCursorToPlane = (normX: number, normY: number) => {
    ...
    return {
        ...
        handlePointerMove(normX: number, normY: number, prevNormX: number, prevNormY: number) {
            if (state !== 'shaking') return;
...
export function registerDiceCupController(controller: any) {
